...
class Solution {
    public String toGoatLatin(String S) {
        ...
        for(int i = 0 ; i < dealStr.length ; i++){
            char first = dealStr[i].charAt(0);
            if(list.contains(first)){
                sb.append(dealStr[i]).append("ma");
            }else{
                sb.append(dealStr[i].substring(1)).append(first).append("ma");
            }

            for(int j = 0 ; j < i + 1; j++){
                sb.append("a");
            }
            if(i < dealStr.length - 1){
                sb.append(" ");
            }
