...
class Solution {
    public String maskPII(String S) {
        ...
        if(atIndex > 0){        //电子邮箱
            ...
            result.append(name1.substring(0,1));
            result.append("*****");
            result.append(name1.substring(name1.length()-1,name1.length()));
            result.append(S.substring(atIndex,S.length()));

        }else{                  //手机号码
            String telnum = S.replace("+","").replace("-","").replace(" ","").replace("(","").replace(")","");
            if(telnum.length() == 10){  //本地号码
                result.append("***-***-");

            }else{                      //国际号码
                result.append("+");
                for(int i = 0 ; i < telnum.length() - 10 ; i++){
                    result.append("*");
                }
                result.append("-***-***-");
            }
            result.append(telnum.substring(telnum.length()-4,telnum.length()));
        }
